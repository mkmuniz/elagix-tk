...
- **Layer B covers 15 filters, still not the whole long tail.** Since 2026-09-24: `npm`/`pnpm`/`yarn` install, `pip`/`pip3` install, `docker` images/pull/build, `dotnet` build/test/run, cargo's stderr progress, `go test` + go's download chatter, `git branch`, `terraform plan`. The `go` and `terraform` filters are unit-tested only (no Go/Terraform on the dev machine). Still no rule for: `rg`, `make`, `jq`, `poetry`, `uv`, `nx`, `turbo`, `pre-commit`, `eslint`, among others. Checked and deliberately left without a rule (real output, 2026-09-24): `tsc`, `vitest` and `jest` are already compact when stdout isn't a TTY — one line per type error; only failing tests plus a summary — so a filter would save a few dozen bytes and risk hiding a diagnostic. `yarn` with no arguments (= install) isn't matched.
